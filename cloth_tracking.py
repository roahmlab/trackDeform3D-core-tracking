...
from utils.init_visualization import (save_init_visualization_3d_cloth,
...
def process_clip(data, transforms, ee_poses_3d, clip_idx, start_frame, end_frame,
    ...
    for frame_idx in tqdm(range(n_frames), desc=f"  Clip {clip_idx}"):
        ...
        if mode == 'init' and not init_vis_saved and keypoints is not None:
            ...
            save_init_visualization_3d_cloth(
                keypoints=keypoints,
                edges=edges,
    ...
    # ------------------------------------------------------------------
    if len(stored_edges) > 0 and any(success_flags):
        print(f"  Rendering tracking video (sigma={sigma})...")
        node_t = grid_gradient_t(grid_rows, grid_cols)
        anchors = set(getattr(tracker, 'contour_corner_grid_indices', None)
                      or getattr(tracker, 'CORNER_INDICES', []))
        render_tracking_video(clip_dir / 'tracking_cloth.mp4',
...
def main():
    ...
    parser.add_argument('--tail_length', type=int, default=30,
                        help='Trajectory tail length in frames (default: 30)')
    ...
    data = load_chunk_data(chunk_dir, mask_file='fg_masks/masks.npz', mask_key='masks',
    ...
    write_chunk_aggregate(
        ...
        f"Cloth Chunk {args.chunk} Aggregate Summary ({n_clips} clips)",
