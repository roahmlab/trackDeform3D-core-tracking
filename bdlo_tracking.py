"""BDLO (branched DLO) tracking driver — Y-shape topology (2 branch, 4 leaf nodes).
...
# NOTE: deform_with_hands monkeypatches `bdlo_tracking.WireTracker`; process_clip
# must stay DEFINED IN THIS MODULE and instantiate the module-global name.
...
def process_clip(data, transforms, ee_poses_3d, clip_idx, start_frame, end_frame,
    ...
    for local_idx, global_idx in enumerate(tqdm(range(start_frame, end_frame), desc=f"    Clip {clip_idx}")):
        ...
        # Reference cloud for the post-hoc evaluation.  NOTE (preserved quirk):
        # unlike the DLO driver, the EE positions are NOT added to the reference
        # cloud here (the historical `len(ee_pos) > 10000` gate was never true).
    ...
    if stored_edges and any(success_flags):
        ...
        anchors = set(range(tracker.reference_n_branch)) | set(ee_leaves)
        masks_clip = (data['masks'][start_frame:end_frame]
                      if data['masks'] is not None else None)
        render_tracking_video(clip_output_dir / 'tracking_bdlo.mp4',
...
def main():
    ...
    data = load_chunk_data(chunk_dir, max_frames=600)
    transforms = load_transforms(calib_dir)

    print(f"  Color: {data['color'].shape}")
    print(f"  Depth: {data['depth'].shape}")
    print(f"  BDLO masks: {data['masks'].shape if data['masks'] is not None else 'None'}")
